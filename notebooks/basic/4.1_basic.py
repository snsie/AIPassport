...
OUTCOME_FRAMING = (
    "Each record here is a row of numbers and one yes/no answer, and the job is to predict the answer from "
    "the numbers. That is the simplest kind of machine learning problem there is. It is also the shape of "
    "most benchtop data: swap these columns for an assay readout or an expression matrix and the same "
    "pipeline runs unchanged, because the pipeline never needs to know what the columns mean."
)
...
if activity == ACTIVITIES[0]:
    ...
    with split_cols[1]:
        ...
        st.markdown(
            "Toggle `StandardScaler()` and watch a single observation's row. It centres every feature at "
            "mean 0 with unit variance."
        )
        ...
        if apply_scaler:
            ...
            st.warning(
                ...
                "integers are bigger — not because it carries more signal. The same thing happens to any "
                "assay whose units are larger than its neighbours'."
            )

        st.dataframe(
            pd.DataFrame([display_data], columns=FEATURES, index=["Observation 0"]).style.format(
                "{:.3f}"
            ),
            width="stretch",
        )
...
if activity == ACTIVITIES[1]:
    ...
    st.markdown(
        "Set the feature values. The tree routes them through the thresholds above and returns a class and "
        "a probability."
    )
    ...
    with st.expander("Reveal expected insights"):
        st.write(
            ...
            "outcome in this dataset. That ranking is a hypothesis about the underlying mechanism — one "
            "you would then go and test."
        )
...
if activity == ACTIVITIES[3]:
    ...
    if priority == "Interpretability":
        st.info(
            "**Ship the tree.** When the model is meant to generate a testable hypothesis, being able to "
            "read the rule *is* the result. An unreadable model gives you a number, not a mechanism."
        )
