...
if audit == AUDITS[0]:
    ...
    with col1:
        ...
        st.markdown(
            "**Audit finding:** low comprehension. Participants may feel alienated or coerced. A signature "
            "obtained this way is legally valid and ethically thin."
        )
    ...
    with col2:
        ...
        if literacy == "Medical jargon":
            ...
            st.success(
                f"**Revised text:** 'We are asking for your permission to use your {SAMPLE_TERM} to help "
                "researchers understand disease. You can say no without affecting your care. You can also "
                "withdraw later. We want you to be a partner in this science.'"
            )
            st.markdown("**Audit result:** compliant. The participant can make an informed choice.")
...
if audit == AUDITS[1]:
    ...
    with c2:
        ...
        s3 = st.checkbox(
            ...
            help="Transportation or mobile clinics, removing access barriers.",
    ...
    if current >= goal:
        st.success(
            f"**Audit result: PASS.** The selected strategies reach a representative sample of "
            f"{DATASET_NAME} ({current}% vs. a {goal}% target)."
        )
    else:
        st.warning(
...
if audit == AUDITS[2]:
    ...
    st.markdown(
        f"""
    Protecting {SUBJECT_TERM.lower()} privacy is a moral obligation before it is a legal one. The protocol
    currently lacks depth. Select the security layers needed to protect the {SAMPLE_TERM} data — you must
    reach a **secure** rating to pass.
    """
    )
    ...
    st.caption(
        ...
        "against re-identification from a rare combination of attributes."
    )
...
if audit == AUDITS[3]:
    ...
    st.markdown(
        """
    **The case:** radiologists label chest X-rays as **cancerous (1)** or **benign (0)**. A model trained on
    inconsistent labels learns the inconsistency. Three things to establish:

    1. **Quantify agreement** — the intraclass correlation coefficient (ICC).
    2. **Find the ambiguous cases** — the images worth a second read.
    3. **Decide how many labellers you need** — because each one costs money.
    """
    )

    c1, c2, c3 = st.columns(3)
    num_samples = c1.slider("Number of X-rays", 50, 1000, 200, key="m3_icc_samples")
    raters = c2.slider("Number of radiologists", 2, 10, 5, key="m3_icc_raters")
    disagreement_rate = c3.slider(
        "Disagreement rate",
        0.0,
        0.5,
        0.2,
        help="Probability that any one radiologist departs from the underlying truth.",
        key="m3_icc_disagreement",
    )

    @st.cache_data
    def simulate_radiologist_annotations(n, n_raters, rate):
        """Simulated annotations. Ground truth is hidden from the raters, as in reality."""
        rng = np.random.default_rng(42)
        ground_truth = rng.integers(0, 2, n)

        data = {"Xray_ID": np.arange(1, n + 1)}
    ...
    annotation_data, ground_truth = simulate_radiologist_annotations(num_samples, raters, disagreement_rate)
    rating_cols = [c for c in annotation_data.columns if c.startswith("Radiologist_")]
    ...
    with col_dist:
        ...
        dist_df = pd.DataFrame(
            {
                "Diagnosis": np.where(
                    numeric_data.values.flatten() == 1, "Cancerous (1)", "Benign (0)"
                )
            }
        )
    ...
    st.markdown(
        "The X-rays with the highest variance across readers — split decisions. These are the candidates "
        "for expert re-review, and the cheapest available improvement to your dataset."
    )
    ...
    @st.cache_data
    def accuracy_by_radiologist_count(n, n_raters, rate):
        annotations, truth = simulate_radiologist_annotations(n, n_raters, rate)
        cols = [c for c in annotations.columns if c.startswith("Radiologist_")]
        accuracies = []
        for i, num in enumerate(range(1, len(cols) + 1)):
            y_consensus = (annotations[cols[:num]].mean(axis=1) > 0.5).astype(int)
            # Stand-in for image embeddings: features that track the consensus label, plus noise
        ...
        return pd.DataFrame(
            {"Count of radiologists": list(range(1, len(cols) + 1)), "Model accuracy": accuracies}
        )

    with st.spinner("Training one model per annotator count..."):
        results_df = accuracy_by_radiologist_count(num_samples, raters, disagreement_rate)

    fig3 = px.line(
        results_df,
        x="Count of radiologists",
        y="Model accuracy",
        markers=True,
    ...
    st.info(
        """
    **Interpretation**
    * **Diminishing returns.** The curve flattens. Its elbow is your answer to "how many readers can we
      afford to stop at".
    * **Noise reduction.** More readers means a more stable consensus, which means better training data —
      up to the point where the remaining error is in the images, not the readers.
    """
    )
...
if audit == AUDITS[4]:
    ...
    st.markdown(
        """
    Your data can be consented, representative, secure, and reliably labelled — and still be useless to a
    collaborator, because it speaks only your institution's dialect. The **OMOP Common Data Model** is the
    fix: source values are replaced with standard **concept IDs** that mean the same thing everywhere.

    Below, synthetic EHR source records are transformed into OMOP `person` and `condition_occurrence`
    tables. Watch what is removed and what is standardized.
    """
    )

    c1, c2 = st.columns(2)
    num_patients = c1.slider("Number of patients", 5, 100, 10, key="m3_omop_n")
    seed_value = c2.number_input(
        "Random seed", value=123, help="Fixed for reproducibility.", key="m3_omop_seed"
    )

    @st.cache_data
    def generate_ehr_source_records(n, seed):
        fake = Faker()
        Faker.seed(seed)
        random.seed(seed)
    ...
    source_data = generate_ehr_source_records(num_patients, seed_value)
    ...
    @st.cache_data
    def generate_ehr_conditions(person_ids, seed):
        fake = Faker()
        Faker.seed(seed)
        random.seed(seed + 1)
    ...
    st.markdown(
        "Diagnoses arrive as ICD-10 source codes and are mapped to OMOP standard concepts. A query written "
        "against `condition_concept_id = 201826` finds type-2 diabetes at a site that codes in SNOMED too."
    )
    st.dataframe(
...
- De-identification is one layer of four, and the weakest one on its own.
